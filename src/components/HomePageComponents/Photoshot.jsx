...
function Photoshot() {
  ...
  const hoverElem = [
    ...
    { element: "Meetings & Evetns",id:'element4' },
  ];
  const images = {
    ...
    <div className="py-24 flex  justify-center items-center bg-gray-100 lg:px-10 2xl:px-44">
      ...
      <div className="flex flex-col lg:flex-row gap-14 lg:gap-0">
        ...
        <div className="md:px-10 xl:w-3/5 order-1">
          <img src={images[hoverElement]} alt="" className={`transition-all ease-out duration-500  ${elementIndex === 0 || elementIndex === 2 ? 'translate-y-7 scale-95': ''}`}/>
          <img src="images/party3.avif" alt="" className={`${hoverElement && 'hidden opacity-0'}`}/>
        </div>
